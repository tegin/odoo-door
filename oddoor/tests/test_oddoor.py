...
class TestOddoor(SavepointCase):
    ...
    def setUpClass(cls):
        ...
        cls.group_1 = cls.env['oddoor.group'].create({
            'name': 'Group 1'
        })
        cls.group_2 = cls.env['oddoor.group'].create({
            'name': 'Group 2'
        })
        cls.lock = cls.env['oddoor.lock'].create({
            'name': 'Lock',
            'group_ids': [(4, cls.group_1.id)],
        })
        ...
        cls.key_2 = cls.env['oddoor.key'].create({
            'name': 'Key 2',
            'group_ids': [(4, cls.group_2.id)]
        })

    def test_allowed_access(self):
        self.assertTrue(self.lock.check_access_unique_virtual_key(
            self.key_1.unique_virtual_key))

    def test_not_allowed_access(self):
        self.assertFalse(self.lock.check_access_unique_virtual_key(
    ...
    def test_inheritance_loop(self):
        group = self.env['oddoor.group'].create({
            'name': 'Group 1 0'
        })
    ...
    def test_lock_find_keys(self):
        ...
        now = Datetime.from_string(Datetime.now())
        key.expiration_date = Datetime.to_string(
            now + timedelta(hours=-1)
        )
        result = self.lock.get_virtual_keys()
        self.assertEqual(len(result), 1)
